...
if __name__ == '__main__':
    ...
    for i in range(nbusses):
        busloc = busdata['Siri']['ServiceDelivery']['VehicleMonitoringDelivery'][0]['VehicleActivity'][i]['MonitoredVehicleJourney']['VehicleLocation']
        next_stop = busdata['Siri']['ServiceDelivery']['VehicleMonitoringDelivery'][0]['VehicleActivity'][i]['MonitoredVehicleJourney']['OnwardCalls']['OnwardCall']
        if DEBUG:
            print(next_stop)
            print('Bus %d is at latitude %f and longitude %f' % (i, busloc['Latitude'], busloc['Longitude']))

        print("%f,%f," % (busloc['Latitude'], busloc['Longitude']), end='', file=of)
